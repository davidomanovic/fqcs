...
def _as_spin_restricted_igcr_ansatz(ansatz: object, *, order: int | None = None):
...
def relabel_igcr_diagonal_coefficients(
    ...
    from xquces.gcr.canonical import IGCRDiagonalCoefficients
    ...
    eta_old = {
        idx: value for idx, value in zip(diagonal.eta_indices, diagonal.eta_vector())
    }
    eta_values = np.asarray(
        [
            eta_old[tuple(sorted((int(old_for_new[p]), int(old_for_new[q]))))]
    ...
    rho_old = {
        idx: value for idx, value in zip(diagonal.rho_indices, diagonal.rho_vector())
    }
...
def transport_igcr2_ansatz_orbitals(ansatz: object, basis_change: np.ndarray):
...
def transport_igcr3_ansatz_orbitals(ansatz: object, basis_change: np.ndarray):
    return transport_legacy_igcr_ansatz_orbitals(
        ansatz,
        basis_change,
        order=3,
...
def transport_igcr4_ansatz_orbitals(ansatz: object, basis_change: np.ndarray):
    return transport_legacy_igcr_ansatz_orbitals(
        ansatz,
        basis_change,
        order=4,
